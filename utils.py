...
class HIMEX(HEDGE): # note, events in mobius are counted in opposite direction from HEDGE (1 is last event in mobius, 0 is first event in HEDGE) 
    def __init__(self, model, data, baseline=None, pruning_idx=0, nsamples=256):
        super().__init__(model, data, baseline=baseline, pruning_idx=pruning_idx)
        explainer = ConsecutiveMobiusKernel(model, baseline, 42)
        self.shap_values = explainer.shap_values(data, nsamples=nsamples, pruning_idx=pruning_idx) 
